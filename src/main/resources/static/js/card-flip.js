...
(() => {
  // 角色簡要提示（用於翻面的標題/備援）
  const roleTips = {
    "偵查官": { title: "偵查官", text: "每回合開始時可查看成功/失敗各自的數量。（每回合可用）" },
    ...
    // 英文鍵（保險）
    "engineer": { title: "偵查官", text: "每回合開始時可查看成功/失敗各自的數量。（每回合可用）" },
  ...
  const roleDetails = {
    ...
    "medic": {
      ...
      effects: [
        "指定一名玩家保護。若該玩家下回合出戰，則不受邪惡技能影響",
        "若保護到正方成功數+1,反之保護到反方成功-1（整場限一次）"
      ],
      tips: [
        "優先保護你最信任且常被鎖定的人。",
        "若誤保壞人會扣分，請謹慎決策。"
    ...
    "lurker": {
      ...
      effects: [
        "可以在提交資源卡時決定替換一張卡片的屬性",
        "成功卡變為失敗卡,反之亦然(本技能可以使用一次)。"
      ],
      tips: [
        "搭配隊友出牌節奏，創造出其不意的逆轉。",
        "別浪費在勝負已定的回合。"
